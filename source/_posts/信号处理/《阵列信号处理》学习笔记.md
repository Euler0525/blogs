---
title: 《阵列信号处理》学习笔记
description: '梳理阵列信号处理中的协方差建模与高分辨 DOA 估计算法，记录 MUSIC、ROOT-MUSIC、最大似然和 ESPRIT 等方法的核心思路。'
tags:
  - 阵列信号处理
categories: 信号处理
mathjax: true
...
=\operatorname{tr} \left [ \mathbf{P}_A^{\perp} \left( \sum_{t = 1}^L \mathbf{x}(t)\mathbf{x}^{\dagger}(t) \right) \right]\\
$$

最后，DML 优化的目标是

$$
\arg\min_{\theta} \operatorname{tr} [P_{A}^{\perp} \hat{R}_x]
$$
...
&= -ML\ln\pi -L\ln|\mathbf{R}_x| - L\cdot \operatorname{tr} [\mathbf{R}_x^{-1}\hat{\mathbf{R}_x}] \\
\end{aligned}
$$
