---
title: 红米 AX6000 路由器刷机流程记录
description: '记录红米 AX6000 从官方固件解锁 SSH、刷入 U-Boot 与第三方固件，到配置 ShellClash、OpenClash、Python 和校园网自动登录脚本的流程。'
tags:
  - 路由器
  - OpenWrt
  - 红米
...
> 个人在使用 Rule 代理时出现了无法 SSH 连接的问题，解决方案是使用自定义规则，将该端口加入 Direct 模式中
>
> ```shell
> # 规则设置 → 勾选自定义规则, 并添加下面内容
> rules:
> - DST-PORT,22,DIRECT
> ```
>
> 配置 OpenClash 代理后，如果使用 `bitsrun` 脚本实现自动登陆校园网，会报错 `bitsrun login error: [Errno 11002] getaddrinfo failed`
>
> 需要补充下面自定义规则
>
> ```shell
...
> - DST-PORT,22,DIRECT
> 
> - IP-CIDR,10.0.0.55/32,DIRECT,no-resolve
> - DOMAIN-SUFFIX,bit.edu.cn,DIRECT
> 
