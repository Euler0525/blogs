---
title: 麦克斯韦方程组
description: '从梯度、散度、旋度等矢量分析基础出发，推导麦克斯韦方程组并解释电磁场与电磁波传播的物理意义。'
tags:
  - 麦克斯韦方程组
  - 电磁波
categories: 通信
...
\text{其中},&\sqrt{\Delta x^2 + \Delta y^2} = l\\
\text{则 }&\lim_{l\rightarrow 0}\dfrac{s(x+\Delta x,y+\Delta y)-s(x,y)}{l} = \dfrac{\partial s}{\partial x}\cos\alpha + \dfrac{\partial s}{\partial y}\sin\alpha,\quad \alpha \text{ 为梯度与 } x \text{ 轴夹角}
\end{aligned}
$$
...
\text{则 }&\oiint_S(j+\varepsilon_0\dfrac{\partial E}{\partial t})\cdot dS = 0
\end{aligned}
$$
