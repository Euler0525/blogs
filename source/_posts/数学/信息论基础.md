---
title: 信息论基础
description: '梳理熵、互信息、KL 散度、数据处理不等式与 Fano 不等式，并由大数定律和渐进均分性质引出典型集及信源编码。'
tags:
  - 熵
  - 相对熵
...
> 1. 概率：$q(x)$ 表示认为 $x$ 发生的可能性是多少；
> 2. 信息：真正发生 $x$ 后 $-\log{q(x)}$ 表示这个结果的意外程度；
> 3. 交叉熵：真实世界不断产生数据 $x\sim p$，平均的意外程度；$h(p, q)=\mathbb{e}_{p}[-\log{q(x)}]$，表示按照模型 $q$ 面对真实的世界 $p$，长期平均有多么意外；
> 4. 熵：如果模型完美 $p=q$，仍然存在 $h(p)$，这是由于真实世界本身的随机性；
> 5. KL 散度：$D_{K}(P||Q)=H(P,Q)-H(P)$，排除真实世界本身的不可预测性之后，由于模型错误产生的额外意外；
