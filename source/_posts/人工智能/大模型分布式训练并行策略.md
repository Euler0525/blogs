...
>
> $$
> \nabla_{\theta} L = \dfrac{1}{B} \sum_{j = 1}^{B} \nabla_{\theta}\mathit{l}(x_j;\theta)
> $$
>
> 现在用 D 张 GPU，每张 GPU 拿到 $B_{\text{local}}=\dfrac{B}{D}$ 个样本，第 $i$ 张 GPU 的局部梯度为
>
> $$
> g_i = \dfrac{1}{B_{\text{local}}} \sum_{x_j\in \mathcal{B}_i} \nabla_{\theta}\mathit{l}(x_j;\theta)
> $$
>
> 然后对所有的 GPU 的梯度求平均
>
...
> &= \dfrac{1}{D} \sum_{i = 1}^D \dfrac{1}{B_{\text{local}}} \sum_{x_j\in \mathcal{B}_i} \nabla_{\theta}\mathit{l}(x_j;\theta)\\
> &= \dfrac{1}{B} \sum_{j = 1}^{B} \nabla_{\theta}\mathit{l}(x_j;\theta)
> \end{aligned}
> $$
>
