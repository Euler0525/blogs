---
title: 支持向量机
description: '从线性 SVM 的几何间隔出发建立优化问题，整理约束、拉格朗日形式和支持向量机的求解流程。'
tags:
  - 最优化
  - Lagrange乘数法
  - SVM
...
如图，数据显然是线性可分的，这些将它们分类的直线称为 **决策面**，每个决策面对应一个线性分类器。但是将它们分开的直线显然不止一条。目前 $H_2$ 和 $H_3$ 的分类效果相同，但如果再增加一个点（在 $H_2$ 和 $H_3$​之间），就会出现分类错误。
