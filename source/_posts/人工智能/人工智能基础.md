---
title: 人工智能基础
description: '整理人工智能与机器学习的基础概念，覆盖频率学派、贝叶斯思想、梯度下降、线性回归、泛化与正则化。'
tags:
  - 线性回归
  - 梯度下降
  - 最大似然估计
...
线性回归中 $\theta$ 的闭合形式可以写成 $\hat{\theta}=(X^TX)^{-1}X^Ty$，这个式子的具体含义整理如下：

### 欧几里得范数

在探究这个式子的含义前，先无脑地推导一下：从解方程组 $y=X\theta+\varepsilon$ 开始，即
...
&= (y-X\theta)^T(y-X\theta) = y^Ty+\theta^TX^TX\theta-y^TX\theta-\theta^TX^Ty\\
&= y^Ty+\theta^TX^TX\theta-2\theta^TX^Ty\quad\Leftarrow\theta(\theta^TX^Ty)_{1\times1}=(\theta^TX^Ty)^T = (y^TX\theta)_{1\times1}\\

\end{aligned}
$$

接下来，令 $\dfrac{\partial(\varepsilon^T\varepsilon )}{\partial \theta}=0$，即

$$
\frac{\partial(\varepsilon^T\varepsilon )}{\partial \theta}= 2X^TX\theta-2X^Ty = 0
$$
...
误差 $\varepsilon=y-\hat{y}=y-X\theta$，当它与 $X$ 正交时，误差是最小的，即
...
&= \underbrace{(1 - \eta\lambda)}_{\text{权重衰减}}\theta - \eta \nabla \mathcal{J}(\theta)\\
\end{aligned}
$$
